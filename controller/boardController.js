...
exports.getAllArticle = (req, res) => {
    ...
    const query = `SELECT *, (SELECT COUNT(*) FROM board_comment WHERE board_id = b.board_id) AS cmt_cnt FROM board b LEFT OUTER JOIN (SELECT student_id, name FROM student) s USING (student_id) ${addtionalQuery} ORDER BY date DESC LIMIT 15`;
...
exports.getArticle = (req, res) => {
    ...
    const query = "SELECT *, (SELECT COUNT(*) FROM board_comment WHERE board_id = b.board_id) AS cmt_cnt FROM board b LEFT OUTER JOIN (SELECT student_id, name FROM student) s USING (student_id) WHERE board_id = ?;";
...
exports.getArticleComment = (req, res) => {
    const { articleID } = req.query;
    const query = "SELECT * FROM board_comment c LEFT OUTER JOIN (SELECT student_id, name FROM student) s USING (student_id) WHERE board_id=?";
    db.query(query, [articleID], (err, results) => {
        if (err) {
            console.error("게시글 가져오는 중 오류:", err);
            res.status(500).send("오류");
